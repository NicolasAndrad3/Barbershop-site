...
const Schedule = () => {
  ...
  return (
    ...
    >
      {/* Header (desktop intact; mobile tipografia/spacing menores) */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 sm:px-6 py-4 sm:py-5 border-b">
        <div>
          <h2 className="text-base sm:text-lg font-semibold text-gray-900">
            Weekly Schedule
          </h2>
          <p className="text-xs sm:text-sm text-gray-500">August 19–24</p>
        </div>
      ...
      {/* Grid: desktop 100% igual; mobile com overflow-x para não esmagar */}
      <div className="relative">
        <div className="overflow-x-auto sm:overflow-visible">
          {/* min-w só no mobile; em sm+ volta ao normal */}
          <div className="grid grid-cols-6 divide-x h-[560px] sm:h-[640px] relative px-2 sm:px-0 min-w-[840px] sm:min-w-0">
            {[
              {
                day: "Mon",
                date: "19",
              ...
              <div key={i} className="relative px-3 sm:px-4 py-3 border-gray-200">
                {/* Day header */}
                <div className="mb-2">
                  <h3 className="text-xs sm:text-sm font-semibold text-gray-500 uppercase tracking-wider">
                    {dayData.day}
                  </h3>
                  <p className="text-xl sm:text-2xl font-bold text-red-500">{dayData.date}</p>
                </div>
                  ...
                  {dayData.events.length > 0 ? (
                    dayData.events.map((event, idx) => (
                      <div
                        ...
                        className="absolute border-l-4 border-red-500 pl-2 sm:pl-3"
                      >
                        <p className="text-xs sm:text-sm font-medium text-red-500">
                          {event.time}
                        </p>
                        <p className="text-xs sm:text-sm text-gray-800">{event.title}</p>
                      </div>
                    ))
                  ) : (
                    <p className="text-xs sm:text-sm text-gray-400 italic mt-4">
                      No appointments
                    </p>
                  )}
                </div>
